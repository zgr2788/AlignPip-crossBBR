...
9. Fastq files should be downloaded.gz format, the `downloadTable{Layout}.sh` scripts are highly recommended if Aspera Connect is installed. If not, the `fastqWrite.sh` script should be manually configured. With Aspera installed, do the following:
